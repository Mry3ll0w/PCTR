import java.util.concurrent.*;
  ...
  public void run(){
    switch(tipoProceso){
      case 0:{  //proceso sucesion
           ...
           //for(int i=0; i<20; i++)
             {
             try{sem1.acquire();}catch(InterruptedException e){}
             s = S0 + S1;
             sem2.release();
             S0 = S1;
             S1 = s;
       ...
       case 1: {//proceso suma
           ...
           ///for(int i=0; i<20; i++)
           {
             try{sem2.acquire();}catch(InterruptedException e){}
             try{sem2.acquire();}catch(InterruptedException e){}
             suma = suma + s;
             //por que dos release? release desbloquea procesos mientras los haya en la cola, 
  ...
  public static void main(String[] args)throws Exception{
    ...
    sucesion.start(); suma.start();
    sucesion.join(); suma.join();
  }
}//class