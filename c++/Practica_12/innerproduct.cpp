...
float a[dim];               // Componente prod escalar
float b[dim];               // Componente prod escalar
double p[4] = {0, 0, 0, 0}; // Acumulador de soluciones parciales.
double pEscalar = 0;

void f(long ini, long fin) // Realiza el prod escalar de forma secuencial
{
...
void g(int id, long ini, long fin) // Realiza el prod escalar usando la división del dominio de datos.
{
  for (long i = ini; i <= fin; i++)
  {
    p[id] = p[id] + (a[i] * b[i]);
